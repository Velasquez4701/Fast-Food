...
VALUES	('Hamburguesa Deluxe', 1, 9.99),
		('Cheeseburger', 1, 7.99),
...
VALUES	('Sucursal Central', '1234 Main St'),
...
VALUES	('Cliente Uno', '1000 A Street'),
...
INSERT INTO TiposPago (Descripcion)
VALUES	('Efectivo'),
		('Tarjeta de Crédito'),
		('Tarjeta de Débito'),
		('PayPal'),
...
INSERT INTO Mensajeros (Nombre, EsExterno)
VALUES	('Mensajero Uno', 0),
		('Mensajero Dos', 1),
		('Mensajero Tres', 0),
		('Mensajero Cuatro', 1),
		('Mensajero Cinco', 0),
...
INSERT INTO Ordenes (ClienteID, EmpleadoID, SucursalID, MensajeroID, TipoPagoID, OrigenID, HorarioVenta, TotalCompra, KilometrosRecorrer, FechaDespacho, FechaEntrega, FechaOrdenTomada, FechaOrdenLista) 
VALUES	(1, 1, 1, 1, 1, 1, 'Mañana', 50.00, 5.5, '2023-01-10 08:30:00', '2023-01-10 09:00:00', '2023-01-10 08:00:00', '2023-01-10 08:15:00'),
...
---------- CONSULTAS BASICAS ----------
-- Consulta 1: Registros unicos
SELECT COUNT(*) AS TotalRegistrosUnicos FROM Ordenes;

-- Consulta 2: Empleados por departamento
SELECT Departamento, COUNT(EmpleadoID) AS NumeroDeEmpleados FROM Empleados GROUP BY Departamento;

-- Consulta 3: Productos por categoria
SELECT CategoriaID, COUNT(ProductoID) AS NumeroDeProductos FROM Productos GROUP BY CategoriaID;

-- Consulta 4: Importacion de Clientes
SELECT COUNT(ClienteID) AS TotalClientes FROM Clientes;

-- Consulta 5: Analisis de desempeño de sucursales
